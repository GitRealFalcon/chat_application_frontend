...
const Sidebar = ({ Chats = [] }) =>{
  ...
  const handleSelect = (chat: Chat): void => {
  ...
  interface ChatMessagesMap {
    [chatId: string]: ChatMessage[] | undefined
  }

  const getLastMessage = (chatId: string): string | undefined => {
    const chatMessages = (messages as ChatMessagesMap | undefined)?.[chatId]
  ...
  return (
    <aside className='flex flex-col border-r border-[#1f2c34] bg-[#111b21] max-[900px]:absolute max-[900px]:w-screen h-screen'>
      ...
      <div className='flex items-center justify-between px-4 py-3'>
        ...
        <img className='hidden max-[900px]:block' onClick={handleClose} src={closeIcon} alt="close" />
